...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{__('common.news')}}</h3>
        ...
        <div class="card-body table-responsive p-0">
            <table class="table table-hover text-nowrap">
                ...
                @foreach($news as $new)
                    <tr>
                        <td>{{$new->id}}</td>
                        <td>{{$new->translate(app()->currentLocale())->title}}</td>
                        <td>{{$new->created_at->format('F j, Y, g:i a')}}</td>
                        <td>
                            <div class="form-group">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" @if($new->status) checked @endif>
{{--                                    <label class="form-check-label" for="is_active">Активна</label>--}}
                                </div>
                            </div>
                        </td>
                        <td>
                            <a href="{{@route('admin.news.edit', $new->id)}}" class="btn btn-warning btn-sm">Edit</a>

                            <form action="{{ route('admin.news.destroy', $new->id) }}" method="POST" style="display:inline-block;">
                                @method('DELETE')
                                @csrf
                                <button data-id="{{$new->id}}" class="btn btn-danger btn-sm btn-delete">Delete</button>
                            </form>
                        </td>
                    </tr>
