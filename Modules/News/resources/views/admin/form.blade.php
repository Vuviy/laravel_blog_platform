...
@section('content')
    <div class="card card-primary">
        ...
        @endif
        @if(isset($news))
            <form method="POST" enctype="multipart/form-data" action="{{route('admin.news.update', ['news' => $news->id])}}" id="new_form">
                @method('PUT')
                @else
                    <form method="POST" enctype="multipart/form-data" action="{{route('admin.news.store')}}" id="new_form">
                        @endif
                        @csrf
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <select name="tags[]" id="tags-select" class="form-control" multiple>
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}"
                                                @if(isset($news) && in_array($tag->id->getValue(), $selectedTagIds ?? []))
                                                    selected
                                            @endif
                                        >
                                            {{ $tag->title }}
                                    ...
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text"
                                                   ...
                                                   class="form-control"
                                                   value="{{ isset($news) ? $news->translate($locale)?->title?->getValue() : '' }}">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div id="editor-{{ $locale }}" style="height: 300px;">
                                                {!! isset($news) ? $news->translate($locale)?->text?->getValue() : '' !!}
                                            </div>

                                            <textarea
                                                name="translations[{{ $locale }}][text]"
                                                id="content-{{ $locale }}"
                                                style="display:none">{{ isset($news) ? $news->translate($locale)?->text?->getValue() : '' }}</textarea>
                                        </div>
                                        ...
                                        <div>
                                            <div class="form-group">
                                                ...
                                                <input type="text"
                                                       ...
                                                       class="form-control"
                                                       value="{{ isset($news) ? $news->translate($locale)?->seoTitle : '' }}">
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text"
                                                       ...
                                                       class="form-control"
                                                       value="{{ isset($news) ? $news->translate($locale)?->seoDescription : '' }}">
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text"
                                                       ...
                                                       class="form-control"
                                                       value="{{ isset($news) ? $news->translate($locale)?->seoKeywords : '' }}">
                                            </div>

                                            <div class="form-group">
                                                <label>{{ __('common.seo_og_image') }}</label>

                                                @if(isset($news) && $news->translate($locale)?->seoOgImage)
                                                    <div>
                                                        <img src="{{ asset('storage/' . $news->translate($locale)->seoOgImage) }}"
                                                             width="200">
                                                    </div>
                                                @endif
                            ...
                            <div class="form-group">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" id="status" name="status"
                                           @if(isset($news) && $news->status) checked @endif>
